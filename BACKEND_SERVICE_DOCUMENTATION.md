...
**Endpoint:** `POST /functions/v1/activate-account`
...
**Endpoint:** `POST /functions/v1/send-auth-link`
...
### 7. `reset-password-token`
Resets password using token from email link.

**Endpoint:** `POST /functions/v1/reset-password-token`
...
1. **JUVLON_API_KEY**
   - Description: API key for Juvlon email service
   - Used by: send-otp, send-auth-link, monthly-interest-credit, process-auto-withdrawals
   - How to set: `supabase secrets set JUVLON_API_KEY=your_api_key`

2. **JUVLON_SMTP_HOST**
   - Description: SMTP host for Juvlon email service
   - Used by: All email-sending functions
   - How to set: `supabase secrets set JUVLON_SMTP_HOST=smtp.juvlon.com`

3. **JUVLON_FROM_EMAIL**
   - Description: Sender email address
   - Used by: All email-sending functions
   - How to set: `supabase secrets set JUVLON_FROM_EMAIL=noreply@goldxusdt.com`
...
**Step 4: Set Secrets**
```bash
supabase secrets set JUVLON_API_KEY=your_api_key
supabase secrets set JUVLON_SMTP_HOST=smtp.juvlon.com
supabase secrets set JUVLON_FROM_EMAIL=noreply@goldxusdt.com
```
...
supabase functions deploy send-auth-link
supabase functions deploy reset-password-otp
supabase functions deploy reset-password-token
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    url := 'https://gtbptywlxhleadgabivi.supabase.co/functions/v1/monthly-interest-credit',
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := 'https://gtbptywlxhleadgabivi.supabase.co/functions/v1/process-auto-withdrawals',
    headers := '{"Content-Type": "application/json", "Authorization": "Bearer YOUR_SERVICE_ROLE_KEY"}'::jsonb
  ) AS request_id;
  $$
);
