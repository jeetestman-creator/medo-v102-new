...
export async function sendEmail({ to, subject, html, from }: SendEmailOptions) {
  ...
  let username = Deno.env.get("ZOHO_SMTP_USER");
  let password = Deno.env.get("ZOHO_SMTP_PASS");
  let host = Deno.env.get("ZOHO_SMTP_HOST") || "smtp.zoho.com";
  let port = parseInt(Deno.env.get("ZOHO_SMTP_PORT") || "465"); 
  ...
  const { data: dbSettings } = await supabase
    .from('settings')
    .select('key, value')
    .in('key', ['zoho_smtp_user', 'zoho_smtp_pass', 'zoho_smtp_host', 'zoho_smtp_port']);
  
  dbSettings?.forEach(s => {
    if (s.key === 'zoho_smtp_user' && s.value) username = s.value;
    if (s.key === 'zoho_smtp_pass' && s.value) password = s.value;
    if (s.key === 'zoho_smtp_host' && s.value) host = s.value;
    if (s.key === 'zoho_smtp_port' && s.value) port = parseInt(s.value);
  });

  if (!username || !password) {
    console.error("SMTP Configuration Missing:", { hasUser: !!username, hasPass: !!password });
    throw new Error("Zoho SMTP credentials not configured. Please set them in Admin Settings.");
  }
