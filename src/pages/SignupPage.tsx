...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: linkData, error: linkError } = await supabase.functions.invoke('send-auth-link', {
        body: { 
      ...
      if (linkError) {
        let errorMsg = linkError.message;
        try {
          if (linkError.context && typeof linkError.context.text === 'function') {
            const contextText = await linkError.context.text();
            try {
          ...
          } else if (linkError.context && typeof linkError.context === 'string') {
            errorMsg = linkError.context;
          }
        } catch (e) {
          console.error('Error parsing error context:', e);
        }
        throw new Error(errorMsg || 'Failed to send activation link');
      }

      toast.success(linkData?.message || 'Activation link sent to your email. Please check your inbox.');
      setStep('otp'); // We reuse 'otp' step but show a success message instead
    } catch (error: any) {
      console.error('Failed to send activation link:', error);
      toast.error(error.message || 'Failed to send activation link');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendLink = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(errorMsg || 'Failed to resend link');
      }

      toast.success(data?.message || 'Activation link resent to your email');
    } catch (error: any) {
            ...
            <div className="space-y-6 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  We've sent an activation link to <span className="text-primary font-medium">{email}</span>. Please click the link to activate your account.
                </p>
              </div>
