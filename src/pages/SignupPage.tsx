...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!rateLimit(`signup-${email}`, 3, 600000)) {
      return;
    }
    ...
    try {
      ...
      if (linkError) {
        let errorMsg = linkError.message || 'Failed to send activation link';
        try {
          ...
          } else if (error && typeof error.context === 'string') {
  ...
  const handleResendLink = async () => {
    ...
    try {
      ...
      if (error) {
        let errorMsg = error.message || 'Failed to resend link';
        try {
        ...
        <Card className="w-full max-w-md border-border">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-center">
              {step === 'form'
                ? 'Join Gold X Usdt and start earning today' : `Enter the 6-digit code sent to ${email}`
              }
            </CardDescription>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Google Sign In - Only show in form step */}
            {step === 'form' && (
              <>
                <div className="relative">
